...
class DrawHandlers(BaseHandler):
    ...
    _service_class = DrawService
    
    # Additional service for prompt optimization
    _gen_service: Optional[GenService] = None
    
    @classmethod
    async def _get_service(cls) -> DrawService:
        """Get DrawService with correct type annotation"""
        service = await super()._get_service()
        return cast(DrawService, service)
