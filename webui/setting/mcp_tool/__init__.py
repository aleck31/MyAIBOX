...
class ToolHandlers:
        ...
        try:
            ...
            try:                
                ...
                try:
                    ...
                    logger.debug("In async context, using sync tool count approach")
                except RuntimeError:
                    # No running loop, we can create one
                    logger.debug("No async context, could create one if needed")
                ...
                if hasattr(tool_provider, '_initialized') and tool_provider._initialized:
                    logger.debug("Universal tool manager is initialized")
                else:
                    logger.debug("Universal tool manager not initialized")
                    
            except Exception as e:
                logger.debug(f"Could not check universal tool manager: {str(e)}")
            ...
            logger.info(f"Refreshed {len(server_data)} MCP servers with tool counts")
            return server_data, f"✓ Loaded {len(server_data)} MCP servers"
            
        except Exception as e:
            logger.error(f"Error refreshing MCP servers: {str(e)}")
    ...
    def _get_server_tool_count(cls, server_name: str) -> int:
        ...
        """
        try:
            # Get tool count directly - ToolProvider uses lazy loading
            all_tools = tool_provider.list_tools()
            mcp_tools = [t for t in all_tools if t.get('type') == 'mcp_server']
            count = len([t for t in mcp_tools if t['name'] == server_name])
            logger.debug(f"Found {count} tools for {server_name}")
            return count
                
        except Exception as e:
            logger.warning(f"Error getting tool count for {server_name}: {str(e)}")
            return 0
    ...
    def get_server_details(cls, servers_data: Any, evt: gr.SelectData) -> Tuple[str, str, str, str, str, str]:
        ...
        """
        try:
            logger.info(f"Selection event: row={evt.index[0]}")
            
            # Handle both DataFrame and list formats
            if servers_data is None:
                logger.warning(f"No server data available")
            ...
            if hasattr(servers_data, 'values'):
                ...
                if hasattr(servers_data, 'empty') and servers_data.empty:
                    logger.warning(f"Server data is empty")
                    return "", "", "", "", "", "✗ No servers available"
                servers_list = servers_data.values.tolist()
            else:
                # It's already a list
                if not servers_data:
                    logger.warning(f"Server list is empty")
                    return "", "", "", "", "", "✗ No servers available"
                servers_list = servers_data
            
            if evt.index[0] >= len(servers_list):
                logger.warning(f"Invalid selection: index={evt.index[0]}, data_length={len(servers_list)}")
            ...
            logger.info(f"Selected server: {server_name}")
        ...
        def reload_task():
            try:
                ...
                try:
                    ...
                    logger.info(f"Tools reloaded successfully after {operation_name}")
                except Exception as e:
                    logger.error(f"Failed to reload tools after {operation_name}: {str(e)}")
                finally:
                    loop.close()
                    asyncio.set_event_loop(None)
            except Exception as e:
                logger.error(f"Error in background tool reload: {str(e)}")
    ...
    def enable_server(cls, server_name: str) -> Tuple[List[List], str]:
        ...
        try:
            ...
            logger.error(f"Error enabling server: {str(e)}")
    ...
    def disable_server(cls, server_name: str) -> Tuple[List[List], str]:
        ...
        try:
            ...
            logger.error(f"Error disabling server: {str(e)}")
    ...
    def get_server_tools(cls, server_name: str) -> Tuple[List[List], str]:
        ...
        try:
            ...
            # Get tools directly - ToolProvider uses lazy loading
            try:
                all_tools = tool_provider.list_tools()
                mcp_tools = [t for t in all_tools if t.get('type') == 'mcp_server']
                
                for tool_info in mcp_tools:
                    if tool_info['name'] == server_name:
                        # This is the server entry, not individual tools
                        # For now, we'll show the server as having 1 "tool" (itself)
                        tools_data.append([
                            server_name,
                            tool_info.get('description', 'MCP Server'),
                            server_name
                        ])
                ...
                logger.error(f"Error initializing or getting tools: {str(e)}")
                return [], f"Error loading tools: {str(e)}"
            
        except Exception as e:
            logger.error(f"Error getting server tools: {str(e)}")
            return [], f"✗ Error loading tools: {str(e)}"
