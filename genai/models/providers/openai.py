...
class OpenAIProvider(LLMAPIProvider):
    ...
    def __init__(self, model_id: str, llm_params: LLMParameters, tools=None):
        ...
        """
        # Explicitly cast to satisfy type checker
        super().__init__(model_id, llm_params, tools)
        # Store with correct type annotation
    ...
    def _convert_message(self, message: LLMMessage) -> Dict:
        ...
        # Return appropriate format based on content complexity
        if len(content_parts) == 1 and content_parts[0].get("type") == "text":
            # Simple text message - use string format per OpenAI SDK
            return {"role": message.role, "content": content_parts[0]["text"]}
        elif content_parts:
            # Multimodal or complex message - use array format per OpenAI SDK
    ...
    def generate_stream(
        ...
        **kwargs
        ...
        try:
            ...
            # Use sync client instead of async client
